...
def prepare_squared_distance_file(data_file,variable_name,N,D,measure_type='dSq',
  ...
  cleanup=False,no_block=True,run_mpi=True,norm=True):
# 
...
################################################################################
  ...
  # yRow,yCol 1-based
  for row in my_row:
    yVal_mine,mask = collect_concat_measure(n,row=row)
    if (norm == True):
      yVal_mine = yVal_mine/mask
    del mask
...
if __name__=="__main__":
  ...
  norm = norm=='True'
  prepare_squared_distance_file(data_file,variable_name,N,D,measure_type,c,h5,transpose,n,nN,sqDist_file,cleanup,no_block,run_mpi,norm)
